...
RUN echo 'kernel.unprivileged_userns_clone=1' > /etc/sysctl.d/00-local-userns.conf && service procps restart
RUN chown root chrome-sandbox && chmod 4755 chrome-sandbox

# add new user to avoid runing as root which is not allowed with chrome-sandbox
RUN groupadd -g 999 quantme && \
    useradd -r -u 999 -g quantme quantme
USER quantme
